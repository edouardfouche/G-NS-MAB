...
/**
  ...
  */
case class ImpCPD3(val stream: Simulator, val reward: Reward, val scalingstrategy: ScalingStrategy, var k: Int) extends BanditUCB {
  ...
  val name = s"ImpCPD3" //TODO: This is the implementation that is closest to the code + some small optimization (unsure)
    ...
    def ChangePointDetectionImproved: Boolean = {
      (0 until narms).foreach { x =>
        if (history(x).map(_._1).slice(ts - 1, history(x).length).sum > 30) { // Let's only care about the arms with enough samples
          if (m > 1) {
            val filteredhistory = history(x).slice(ts - 1, history(x).length).filter(_._1 > 0) // TODO: Take only the rounds that were played, that part is different a bit
            filteredhistory.indices.slice(5, filteredhistory.length - 5).filter(x => x % 10 == 0).foreach { y =>
              //val lprime = epsilon_L_history(mprime)
              val s1 = filteredhistory.map(_._2).slice(0, y).sum
              val n1 = filteredhistory.map(_._1).slice(0, y).sum
              val c1 = math.sqrt((alpha * math.log(phi * math.pow(epsilon, 2))) / (2 * n1))
              val s2 = filteredhistory.map(_._2).slice(y, filteredhistory.length).sum
              val n2 = filteredhistory.map(_._1).slice(y, filteredhistory.length).sum
              val c2 = math.sqrt((alpha * math.log(phi * math.pow(epsilon, 2))) / (2 * n2))
              if ((s1 / n1 + c1) < (s2 / n2 - c2)) {
                //println(Console.BLUE + s"Option1: ImpCPD Change detected at time $t ($tp) on arm $x")
                //println(s"We have B.size =${B.size}, M=$M, m=$m, lprime=$y, L=$L, t=$t")
                //println(s"alpha: $alpha, phi: $phi, epsilon: $epsilon, horizon: $horizon ")
                //println {s"Means are $s1 / $n1 - $c1 > $s2 / $n2 + $c2"}
                //println(s"slicing n1 on ${ts - 1}, ${y - 1}")
                //println(s"slicing n2 on ${y - 1}, ${history(x).length}")
                return true
              }
              if ((s1 / n1 - c1) > (s2 / n2 + c2)) {
